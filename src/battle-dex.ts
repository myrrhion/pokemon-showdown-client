...
const Dex = new class implements ModdedDex {
	...
	resourcePrefix = (() => {
		...
		return `${prefix}//${window.Config ? Config.routes.play : 'play.pokemonshowdown.com'}/`;
	})();

	fxPrefix = (() => {
		if (window.document?.location?.protocol === 'file:') {
			if (window.Replays) return `https://${window.Config ? Config.routes.play : 'play.pokemonshowdown.com'}/fx/`;
			return `fx/`;
		}
